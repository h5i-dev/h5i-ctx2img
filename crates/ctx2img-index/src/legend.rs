...
pub fn build_legend(built: &Built, query: &str, opts: &LegendOptions) -> String {
    ...
    if opts.schema {
        ...
        out.push_str("#   `ctx2img paint <dir>` module source as images · `ctx2img read F103` exact text · `ctx2img read --find <pat>` search\n");
    }
    if !query.is_empty() {
...
mod tests {
    ...
    fn legend_contains_handles_and_bands() {
        ...
        assert!(legend.contains("ctx2img read"), "affordances stated");
        assert!(
