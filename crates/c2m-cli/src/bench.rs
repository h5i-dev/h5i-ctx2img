//! `c2m bench` — localization benchmark: atlas+legend vs text-only legend
...
use c2m_render::{render_png, scene, SceneConfig, VlmTheme};
...
pub fn bench(
    ...
    for (i, task) in tasks.iter().enumerate() {
        ...
        if live {
            ...
            let a_atlas = c2m_eval::live::ask_about_image(&png, &legend, &q, model)?;
            let a_text = c2m_eval::live::ask_text(&format!("{legend_full}\n\n{q}"), model)?;
