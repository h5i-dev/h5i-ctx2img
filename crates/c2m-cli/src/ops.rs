...
pub const FOOTER: &str = "next: `c2m paint <dir>` module source as images · `c2m read F#|S#|path` exact text · `c2m read --find <pat>` search";
...
#[allow(clippy::too_many_arguments)]
/// Walk up from `start` to the enclosing repo root (.git or .c2m marker).
...
fn discover_root(start: &Path) -> PathBuf {
    ...
    loop {
        if cur.join(".git").exists() || cur.join(".c2m").exists() {
            return cur;
        }
        match cur.parent() {
